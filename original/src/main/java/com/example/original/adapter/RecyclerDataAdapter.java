package com.example.original.adapter;
...
import com.example.original.activity.EditActivity;
...
//                        mHelper.openReadLink(); // 打开数据库帮助器的读连接
                        try {
                            MyDBHelper.getInstance(mContext).getPwInfoDao().deleteById(passwordInfoList.get(position1).xuhao);
